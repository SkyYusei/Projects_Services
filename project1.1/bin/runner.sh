...
answer_0() {
	...
	#	python filter.py > output
        javac filter.java
        java filter pagecounts-20150801-000000        
        cat output | sort -nrk 2,2 -o output
}
...
answer_1() {
	...
	wc -l pagecounts-20150801-000000 | awk '{print $1;}'
}
...
answer_2() {
	...
	awk '{sum+=$(NF-1)} END {print sum}' pagecounts-20150801-000000
}
...
answer_7() {
        ...
        awk '$2>2500 {count++} END {print count}' output
}
...
answer_8() {
	...
	awk 'match($1, /^List_of.*episodes$/) {sum+=$2} END {print sum}' output
}
...
answer_9() {
        ...
        a=$(awk '$1 ~ /(2014_film)/ {sum+=$2} END {print sum}' output)
        b=$(awk '$1 ~ /(2015_film)/ {sum+=$2} END {print sum}' output)
        if [ "$a" -gt "$b" ]
        then
                echo "2014"
        else
